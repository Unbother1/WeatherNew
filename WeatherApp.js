...
const conditionIcon = {
  Clear: "🌞", Rain: "☔️", Clouds: "☁️", Snow: "❄️", Thunderstorm: "⛈️",
  Drizzle: "🌧️", Mist: "🌫️", Smoke: "🌫️", Haze: "🌫️", Dust: "🌬️",
  ...
  try {
    ...
    resultBox.innerHTML = `
      <div class="weather-card-output">
        ...
        <p><strong>Temp:</strong> <span id="tempValue">${useCelsius ? tempC + '°C' : tempF + '°F'}</span></p>
        <p><strong>Humidity:</strong> ${data.main.humidity}%</p>
        <p><strong>Wind:</strong> ${data.wind.speed} m/s</p>
        <p><strong>Condition:</strong> ${data.weather[0].main}</p>
        <img class="animated-icon" src="https://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png" alt="weather icon"/>
      </div>
    `;
...
function displayForecast(forecastData) {
  ...
  daily.forEach(day => {
    ...
    const shortDate = date.toLocaleDateString(undefined, { weekday: 'short', day: 'numeric' });
